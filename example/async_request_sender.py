...
class Context:
    ...
    send_timestamp: float = field(default=0)
    ## outpt
...
class AysncRequestSender:
    ...
    async def do_post_request(self, url: str, ctx: Context, chat: bool, extra: Dict[str, Any]):
        ...
        if ctx.send_timestamp != 0:
            print(f"[INFO] Request {ctx.index} will be sent after {ctx.send_timestamp} seconds")
            await asyncio.sleep(ctx.send_timestamp)
        async with aiohttp.ClientSession(timeout=AIOHTTP_TIMEOUT) as session:
            ...
            try:
                ...
                # 捕获 HTTP 层面的错误（连接超时、DNS 失败等）
                logger.error("HTTP request failed: %s", e)
            except Exception as e:
                # 捕获其他意外异常
                logger.error("Unexpected error in do_post_request")
