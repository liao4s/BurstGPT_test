...
class ServerBase(object):
    def __init__(self, model_path, data_path, backend,
        ...
        print("log_path == ", self.log_path, "----"*10)
...
class ServerOnline(ServerBase):
    ...
    def save_log(self):
        ...
        print("[INFO]Saving detail log data")
        self.detail_logger.save("w")

        print("[INFO]Saving server log data")
    ...
    def end_profile(self):
        self.save_log()
        self.monitor.save_gpu_log()
        if len(self.vllm_log_path) != 0:
            os.system(f"mv ./logs/vllm_log/vllm_log.csv {self.vllm_log_path}")
            print("Load vllm log successfully!!")
        print("Os Exit!!")
        os._exit(0)